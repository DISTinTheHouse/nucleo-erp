...
- Body — **reemplaza completo** el plan de programación anterior (no hace merge/append):
  ...
  - **Validación dura de destinos**: `BORDADO`/`REFLEJANTE`/`CORTE_MANGA` sólo se aceptan si el pedido realmente tiene al menos una talla con ese servicio (`lleva_bordado`/`lleva_reflejante`/`lleva_corte_manga=True`) — si no, `400`:
    ...
    ```
    Es todo o nada: si mandas un destino válido junto con uno inválido en el mismo PATCH, no se guarda ninguno. `EMBARQUE`/`APARTADO` son logística genérica y siempre se aceptan, sin importar qué lleve el pedido. Ver `destinos_aplicables` abajo para saber de antemano qué destinos mandar.
...
| Campo                                      | Regla                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| ------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `pedidos`                                  | Solo pedidos con al menos una `PedidoDetalleTalla` con `lleva_bordado=True` **y con saldo pendiente** (si todas sus líneas están cubiertas al 100% por OBs activas, el pedido no aparece) **y que mesa de control ya programó explícitamente hacia `BORDADO`** (`PATCH /api/v1/ventas/pedidos/{id}/programar/`). **Filtro fijo, sin excepción**: un pedido con bordado pendiente pero sin esa programación **no aparece aquí**, aunque tenga todo lo demás en orden — es la forma en que Mesa de Control decide qué se manda a producción. Scope por `empresa` + `sucursales_permitidas()` del usuario.                                                                                                                                                                                                                                                                                                                           |
| `pedidos[].programado`                     | `{cantidad, fecha, usuario_nombre}` leído tal cual de `Pedido.programacion_conf` para el destino `BORDADO` (sumando si hubiera más de una fila programada a ese destino) — **nunca viene `null` en este listado**, porque `null` significa "no programado", y esos pedidos ya no aparecen (ver fila de arriba). Es informativo: la OB se sigue armando igual que siempre, a partir de `detalles[]` (que sale de `PedidoDetalleTalla.lleva_bordado=True`), no de este campo.                                                                                                                                                                                                                                                                                                    |
...
```json
{
  ...
  "detalles": [
    {
      ...
      "puntadas": 8000,
      "posicion_bordado": "F",
      "tipo_servicio_bordado": "Combinado",
      "tipo_servicio_bordado_display": "Combinado (2+ técnicas)",
      "tipos_servicio": ["NUEVO_PONCHADO", "SUBLIMADO"],
      "tipos_servicio_display": [
        { "value": "NUEVO_PONCHADO", "label": "Nuevo ponchado" },
        { "value": "SUBLIMADO", "label": "Sublimado" }
      ],
      "descripcion_servicio": "Logo nítido + sublimado cuello"
    },
    {
      "id": 102,
      "pedido_detalle_id": 3301,
      ...
      "puntadas": 8000,
      "posicion_bordado": "F",
      "tipo_servicio_bordado": "Plano",
      "tipo_servicio_bordado_display": "Bordado Plano",
      "tipos_servicio": ["DTF"],
      "tipos_servicio_display": [{ "value": "DTF", "label": "DTF" }],
  ...
  "resumen_avance": {
    ...
    "puntadas_porcentaje_avance": 80.0,
    "por_detalle": [
      {
        ...
        "puntadas_porcentaje_avance": 100.0,
        "operadores": [
          {
            "usuario_id": 4,
            "usuario_nombre": "María Gómez",
      ...
      },
      {
        ...
        "puntadas_porcentaje_avance": 40.0,
        "operadores": [
          {
            "usuario_id": 4,
            "usuario_nombre": "María Gómez",
...
| Campo                                  | Regla                                                                                                                                                              |
| -------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `pedidos`                              | Igual que en el onboarding de Bordado (ver esa sección): solo pedidos con al menos una `PedidoDetalleTalla` con `lleva_reflejante=True`, con saldo pendiente, **y ya programados hacia `REFLEJANTE`** (`PATCH /api/v1/ventas/pedidos/{id}/programar/`) — filtro fijo, sin excepción. Scope por `empresa` + `sucursales_permitidas()` del usuario. |
| `pedidos[].programado`                 | `{cantidad, fecha, usuario_nombre}` del destino `REFLEJANTE` — nunca `null` aquí, porque sin programación el pedido no aparece (ver fila de arriba). Informativo; no cambia cómo se arma la OR. |
...
| Campo                                  | Regla                                                                                                                                                              |
| -------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `pedidos`                              | Igual que en el onboarding de Bordado (ver esa sección): solo pedidos con al menos una `PedidoDetalleTalla` con `lleva_corte_manga=True`, con saldo pendiente, **y ya programados hacia `CORTE_MANGA`** (`PATCH /api/v1/ventas/pedidos/{id}/programar/`) — filtro fijo, sin excepción. Scope por `empresa` + `sucursales_permitidas()` del usuario. |
| `pedidos[].programado`                 | `{cantidad, fecha, usuario_nombre}` del destino `CORTE_MANGA` — nunca `null` aquí, porque sin programación el pedido no aparece (ver fila de arriba). Informativo; no cambia cómo se arma la OCM. |
